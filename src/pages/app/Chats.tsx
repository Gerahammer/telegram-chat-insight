    ...
    <Link to={`/app/chats/${encodeURIComponent(c.id)}`} className="block">
      <Card className="p-5 h-full hover:border-primary/40 hover:shadow-md transition">
        <div className="flex items-start gap-3">
          {c.photoUrl ? <img src={`${import.meta.env.VITE_API_URL || "https://seahorse-app-47666.ondigitalocean.app"}/api/proxy/image?url=${encodeURIComponent(c.photoUrl)}`} alt={c.title} className="h-10 w-10 rounded-lg object-cover shrink-0" onError={(e: any) => { e.target.style.display="none"; }} /> : <div className="h-10 w-10 rounded-lg bg-primary/10 text-primary flex items-center justify-center shrink-0">
